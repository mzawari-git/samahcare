...
<footer class="pt-16 pb-8 relative overflow-hidden" style="border-top:1px solid var(--glass-border);">
    <div class="max-w-7xl mx-auto px-4 relative z-10">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10 mb-12">
            <div class="lg:col-span-4 text-right">
                <div class="flex items-center gap-3 mb-5 justify-end">
                    ...
                    @else
                        <span class="text-2xl font-extrabold">{{ $siteSettings['site_name'] ?? 'سماح كير ' }}<span style="color:var(--brand-500);">.</span></span>
                    @endif
                </div>
                <p class="text-ink-dim text-sm leading-relaxed mb-6 max-w-sm ml-auto">{{ $siteSettings['site_description'] ?? 'وجهتك لخدمات التجميل والعناية.' }}</p>
            ...
            <div class="lg:col-span-2 text-right">
                ...
                <ul class="space-y-3 text-ink-dim text-sm">
                    @foreach($headerServices->take(5) as $s)
                        <li><a href="{{ route('booking') }}" class="hover:text-ink transition-colors">{{ $s->name_ar }}</a></li>
                    @endforeach
                </ul>
            </div>
            <div class="lg:col-span-2 text-right">
                ...
                <ul class="space-y-3 text-ink-dim text-sm">
                    <li><a href="{{ route('booking') }}" class="hover:text-ink transition-colors">احجزي موعد</a></li>
                    <li><a href="{{ route('blog.index') }}" class="hover:text-ink transition-colors">المدونة</a></li>
                    <li><a href="{{ route('contact') }}" class="hover:text-ink transition-colors">تواصل معنا</a></li>
                    <li><a href="{{ route('terms') }}" class="hover:text-ink transition-colors">الشروط والأحكام</a></li>
                    <li><a href="{{ route('privacy') }}" class="hover:text-ink transition-colors">سياسة الخصوصية</a></li>
                </ul>
            </div>
            <div class="lg:col-span-4 text-right">
        ...
        <div class="pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-sm text-ink-dim" style="border-top:1px solid var(--glass-border);">
