...
<div class="min-h-[80vh] flex items-center justify-center px-4 pt-32 pb-16">
    <div class="w-full max-w-lg bg-white rounded-3xl border border-gray-100 shadow-xl p-8">
        <div class="text-center mb-8">
            ...
            <h1 class="text-2xl font-extrabold text-ink">إنشاء حساب جديد</h1>
            <p class="text-gray-500 text-sm mt-1">انضمي إلينا واستمتعي بتجربة تسوق فريدة</p>
        </div>

        <form action="{{ route('register') }}" method="POST" class="space-y-5">
            @csrf
            <div>
                <label class="block text-sm font-bold text-ink mb-1.5">الاسم الكامل <span class="text-red-400">*</span></label>
                <input type="text" name="name" value="{{ old('name') }}" required class="w-full bg-surface border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all @error('name') border-red-400 @enderror" placeholder="اسمك الكامل">
            ...
            <div>
                ...
                <input type="email" name="email" value="{{ old('email') }}" required class="w-full bg-surface border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all @error('email') border-red-400 @enderror" placeholder="example@email.com">
                @error('email')<p class="text-red-500 text-xs mt-1 flex items-center gap-1"><i class="ph ph-warning-circle"></i> {{ $message }}</p>@enderror
            </div>

            <div>
                <label class="block text-sm font-bold text-ink mb-1.5">رقم الهاتف <span class="text-red-400">*</span></label>
                <input type="text" name="phone" value="{{ old('phone') }}" required class="w-full bg-surface border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all @error('phone') border-red-400 @enderror" placeholder="05XX XXXXXX">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="password" name="password" required class="w-full bg-surface border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all @error('password') border-red-400 @enderror" placeholder="********">
                ...
                <div>
                    ...
                    <input type="password" name="password_confirmation" required class="w-full bg-surface border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all" placeholder="********">
                </div>
            </div>

            <div>
                <label class="block text-sm font-bold text-ink mb-1.5">سؤال الأمان <span class="text-red-400">*</span></label>
                <select name="security_question" required
                    class="w-full bg-surface border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all @error('security_question') border-red-400 @enderror">
            ...
            <div>
                <label class="block text-sm font-bold text-ink mb-1.5">إجابة سؤال الأمان <span class="text-red-400">*</span></label>
            ...
            <button type="submit" class="w-full py-3.5 bg-ink text-white rounded-full font-bold hover:bg-brand-600 transition-all shadow-lg flex items-center justify-center gap-2">
