...
function generate_google_maps_embed($config = null) {
    ...
    $lat  = urlencode($config['latitude']);
    $lng  = urlencode($config['longitude']);
    $zoom = intval($config['zoom']);

    // Map type: Google Maps embed uses t= parameter
    // m=roadmap, k=satellite, h=hybrid, p=terrain
    $typeMap = [
        'roadmap'   => 'm',
        'satellite' => 'k',
        'hybrid'    => 'h',
    ...
    $t = $typeMap[$config['map_type']] ?? 'm';

    // Simple, reliable embed URL — no API key required
    // q=lat,lng pins the exact location; z=zoom; t=map type
    $embedUrl = "https://maps.google.com/maps?q={$lat},{$lng}&z={$zoom}&t={$t}&output=embed";
